...
const DEFAULT_ROSTER = ["Alex", "Maria", "Jordan", "Sam", "Riley", "Casey", "Morgan", "Drew", "Taylor", "Jamie", "Avery", "Quinn", "Reese", "Sage", "Rowan", "Blake", "Cameron", "Dakota", "Emerson", "Finley", "Harper", "Iris", "Jasper", "Kit"];
...
class Fish {
  constructor(W, H, surfaceY, depth, name) {
    ...
    this.heading = (Math.random() < 0.5 ? 0 : Math.PI) + (Math.random() - 0.5) * 0.6;
    this.speed = 0;
  ...
  update(dt, W, H, surfaceY, depth, hook, allFish, schoolOn, now) {
    ...
    // Speed target
    const targetSpeed = this.state === "chase" ? this.maxSpeed * this.aggression : this.cruiseSpeed;
    ...
    if (this.y < surfaceY + 30) this.heading += dt * (this.heading < 0 ? -2 : 2);
    if (this.y > surfaceY + depth - 24) this.heading -= dt * (this.heading < 0 ? -2 : 2);
  ...
  thickness(u) {
    ...
    if (u < 0.12) {
      ...
      const k = u / 0.12;
      return (0.55 + 0.30 * Math.sin(k * Math.PI / 2)) * bH * 0.5;
    } else if (u < 0.30) {
  ...
  draw(ctx, palette, showOutline, lanternPos, showName) {
    ...
    ctx.quadraticCurveTo(rootUpX + tfx * tailReach * 0.35 + tpx * spread * 0.7, rootUpY + tfy * tailReach * 0.35 + tpy * spread * 0.7, upTipX, upTipY);
    // Upper lobe trailing edge into notch
    ctx.quadraticCurveTo(tx + tfx * tailReach * 0.9 + tpx * spread * 0.2, ty + tfy * tailReach * 0.9 + tpy * spread * 0.2, notchX, notchY);
    // Lower lobe leading edge out to lower tip
    ctx.quadraticCurveTo(tx + tfx * tailReach * 0.9 - tpx * spread * 0.2, ty + tfy * tailReach * 0.9 - tpy * spread * 0.2, dnTipX, dnTipY);
    // Lower lobe outer curve back to root
    ctx.quadraticCurveTo(rootDnX + tfx * tailReach * 0.35 - tpx * spread * 0.7, rootDnY + tfy * tailReach * 0.35 - tpy * spread * 0.7, rootDnX, rootDnY);
    ctx.closePath();
    ctx.fill();
    ...
    const ai1 = Math.floor(N * 0.70),
      ai2 = Math.floor(N * 0.84);
    ctx.fillStyle = sp.finRed;
    ctx.beginPath();
    ctx.moveTo(bot[ai1].x, bot[ai1].y);
    const apkx = (bot[ai1].x + bot[ai2].x) / 2 + Math.sin(angs[(ai1 + ai2) / 2 | 0]) * this.bodyH * 0.55;
    const apky = (bot[ai1].y + bot[ai2].y) / 2 - Math.cos(angs[(ai1 + ai2) / 2 | 0]) * this.bodyH * 0.55;
    ...
    const plI = Math.floor(N * 0.52);
    {
      ...
      const baseX = bot[plI].x,
        baseY = bot[plI].y;
      const tipX = baseX - px * this.bodyH * (0.65 + flap) - fx * this.length * 0.04;
      const tipY = baseY - py * this.bodyH * (0.65 + flap) - fy * this.length * 0.04;
      ctx.fillStyle = sp.finRed;
      ctx.beginPath();
      ctx.moveTo(baseX, baseY);
      ctx.quadraticCurveTo(baseX - px * this.bodyH * 0.3 + fx * 6, baseY - py * this.bodyH * 0.3 + fy * 6, tipX, tipY);
      ctx.quadraticCurveTo(baseX - px * this.bodyH * 0.2 - fx * 4, baseY - py * this.bodyH * 0.2 - fy * 4, baseX - fx * 6, baseY - fy * 6);
      ctx.closePath();
      ctx.fill();
    }
    ...
    const g = ctx.createLinearGradient(cx + gpx * this.bodyH, cy + gpy * this.bodyH,
    // top (back)
    cx - gpx * this.bodyH, cy - gpy * this.bodyH // bottom (belly)
    );
    g.addColorStop(0.00, sp.back);
    g.addColorStop(0.28, sp.body);
    g.addColorStop(0.58, sp.flank);
    g.addColorStop(0.82, sp.belly);
    g.addColorStop(1.00, sp.bellyHi);
    ctx.fillStyle = g;
    ctx.fill();
    ...
    const ptipX = pcx + ppx * this.bodyH * (1.2 + finFlap) - Math.cos(pang) * this.length * 0.02;
    const ptipY = pcy + ppy * this.bodyH * (1.2 + finFlap) - Math.sin(pang) * this.length * 0.02;
    ctx.moveTo(pts[pi].x + ppx * this.bodyH * 0.1, pts[pi].y + ppy * this.bodyH * 0.1);
    ctx.quadraticCurveTo(ptipX - Math.cos(pang) * 4, ptipY - Math.sin(pang) * 4, ptipX, ptipY);
    ctx.quadraticCurveTo(pcx + Math.cos(pang) * 6, pcy + Math.sin(pang) * 6, pts[pi].x + ppx * this.bodyH * 0.25 + Math.cos(pang) * 4, pts[pi].y + ppy * this.bodyH * 0.25 + Math.sin(pang) * 4);
    ctx.closePath();
    ctx.fill();
    // Pectoral rays
    ...
    ctx.quadraticCurveTo(pts[gi].x - Math.cos(gAng) * 3, pts[gi].y - Math.sin(gAng) * 3, bot[gi].x, bot[gi].y);
    ctx.stroke();
    ...
    ctx.moveTo(m1x, m1y);
    ctx.quadraticCurveTo(pts[0].x + mfx * this.length * 0.08, pts[0].y + mfy * this.length * 0.08, m2x, m2y);
    ctx.stroke();
    ...
    ctx.quadraticCurveTo(noseX + mfx * this.length * 0.04 - hpx * this.bodyH * 0.18, noseY + mfy * this.length * 0.04 - hpy * this.bodyH * 0.18, noseX + mfx * this.length * 0.06 - hpx * this.bodyH * 0.30, noseY + mfy * this.length * 0.06 - hpy * this.bodyH * 0.30);
    ctx.stroke();
    // long lower barbel — droops with swim phase
    const droop = Math.sin(this.phase * 0.5) * 0.05;
    ctx.beginPath();
    ctx.moveTo(noseX, noseY + 0.5);
    ctx.quadraticCurveTo(noseX + mfx * this.length * 0.05 - hpx * this.bodyH * 0.35, noseY + mfy * this.length * 0.05 - hpy * this.bodyH * 0.35, noseX + mfx * this.length * 0.08 - hpx * this.bodyH * (0.55 + droop), noseY + mfy * this.length * 0.08 - hpy * this.bodyH * (0.55 + droop));
    ctx.stroke();
    // Eye socket
    ...
    if (lanternPos) {
      ...
      if (dl < 240) {
        ...
        ctx.beginPath();
        ctx.ellipse(this.x, this.y, this.length * 0.5, this.bodyH * 0.7, this.heading, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
      }
    }
    ...
    if (showName && this.state !== "eliminating") {
      ...
      ctx.quadraticCurveTo(rectX + w + padX * 2, rectY, rectX + w + padX * 2, rectY + r);
      ctx.lineTo(rectX + w + padX * 2, rectY + 18 - r);
      ctx.quadraticCurveTo(rectX + w + padX * 2, rectY + 18, rectX + w + padX * 2 - r, rectY + 18);
...
function drawSky(ctx, W, H, surfaceY, p, sunX, sunY, t, weather, lightning) {
  ...
  if (weather !== "clear") {
    ...
    for (let i = 0; i < cloudCount; i++) {
      const baseX = (i * 220 + t * (weather === "storm" ? 18 : 8)) % (W + 400) - 200;
      const baseY = surfaceY * (0.18 + i % 3 * 0.18);
      drawCloud(ctx, baseX, baseY, 160 + i % 3 * 80, 28 + i % 2 * 14, p.cloudShade, weather === "storm" ? 0.85 : 0.7);
    }
    ...
    for (let x = 0; x <= W; x += 30) {
      const y = surfaceY - 18 - Math.sin((x + t * 20) * 0.012) * 8 - Math.sin(x * 0.04) * 4;
      ctx.lineTo(x, y);
    }
...
function drawCloud(ctx, x, y, w, h, color, alpha) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    ctx.ellipse(cx, cy, w / 4 + (i % 2 ? 8 : 0), h * 0.7 + (i % 2 ? 4 : 0), 0, 0, Math.PI * 2);
  }
  ctx.fill();
  ctx.restore();
}
function drawDistantTrees(ctx, W, surfaceY, p, t) {
  ...
  for (let x = 0; x <= W; x += 20) {
    const y = surfaceY - 36 - Math.sin((x + 100) * 0.007) * 22 - Math.sin(x * 0.015) * 10;
    ctx.lineTo(x, y);
  }
  ctx.lineTo(W, surfaceY);
  ctx.closePath();
...
function drawRiverbed(ctx, W, H, p, t) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    ctx.beginPath();
    ctx.moveTo(x, H - 10);
    ctx.quadraticCurveTo(x + sway, H - 30, x + sway * 1.6, H - 50 - i % 3 * 8);
    ctx.stroke();
  }
}
function drawFog(ctx, W, H, surfaceY, p, t, density) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    ctx.moveTo(0, y);
    for (let x = 0; x <= W; x += 24) {
      const yo = Math.sin((x + offset) * 0.012 + i) * 6 + Math.sin((x - offset) * 0.04 + i * 1.7) * 3;
      ctx.lineTo(x, y + yo);
    }
    ctx.lineTo(W, y + 20);
    ctx.lineTo(0, y + 20);
...
function drawBoat(ctx, W, surfaceY, t, drift, palette, logoImg, baitImg, lanternFlicker, lanternOn) {
...
function drawBaitBag(ctx, x, y, w, h, baitImg) {
  ...
  if (baitImg && baitImg.complete && baitImg.naturalWidth > 0) {
    ...
    for (let i = 0; i < 6; i++) {
      ctx.beginPath();
      ctx.arc(lx + 3 + i % 3 * (lw / 3), ly + 4 + Math.floor(i / 3) * (lh / 2), 1.4, 0, Math.PI * 2);
      ctx.fill();
    }
  }
...
function drawHook(ctx, x, y, hooked) {
  ...
  const glow = ctx.createRadialGradient(bx, by, baitR * 0.4, bx, by, baitR * 2.2);
  ...
  const baitG = ctx.createRadialGradient(bx - baitR * 0.4, by - baitR * 0.5, baitR * 0.2, bx, by, baitR * 1.1);
  ...
  ctx.beginPath();
  ctx.ellipse(bx - baitR * 0.42, by - baitR * 0.45, baitR * 0.30, baitR * 0.18, -0.6, 0, Math.PI * 2);
  ctx.fill();
  // Soft small secondary highlight
  ctx.fillStyle = "rgba(255,240,210,.45)";
  ctx.beginPath();
  ctx.arc(bx + baitR * 0.25, by - baitR * 0.6, baitR * 0.10, 0, Math.PI * 2);
  ctx.fill();
  // Drip running down
...
function drawLanternBeam(ctx, W, H, lanternPos, surfaceY, flicker) {
  ...
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  const lg = ctx.createRadialGradient(lanternPos.x, lanternPos.y, 4, lanternPos.x, lanternPos.y, 180);
  ...
  ctx.fill();
  const g = ctx.createLinearGradient(lanternPos.x, lanternPos.y, lanternPos.x, surfaceY + 40);
...
function App() {
  ...
  const rosterArr = useMemo(() => roster.split("\n").map(s => s.trim()).filter(Boolean), [roster]);
  ...
  const startTournament = useCallback(() => {
    ...
    const depthFactor = t.waterDepth === "shallow" ? 0.85 : t.waterDepth === "very-deep" ? 1.15 : 1.0;
  ...
  }, [rosterArr, t.waterDepth, t.sound, t.volume, t.weather, t.timeOfDay, t.rain, t.lantern]);
  ...
  }, [started, t.autoRound, t.autoDelay, hookBusy, aliveCount, winner, releaseRod]);
  ...
  useEffect(() => {
    ...
    function tick(now) {
      ...
      const baseFlicker = 0.86 + Math.sin(s.time * 11) * 0.05 + (Math.random() - 0.5) * 0.06;
      s.lanternFlicker = t.lantern ? Math.max(0.5, Math.min(1.05, baseFlicker)) : 0;
      ...
      if (s.lightningCooldown <= 0 && t.weather === "storm") {
        ...
        if (window.fishAudio) {
          ...
          setTimeout(() => window.fishAudio.playThunder(), 350 + Math.random() * 600);
        }
      }
          ...
          if (d < 6) {
            if (hk.hooked) {
              ...
              if (remaining.length === 1) {
                ...
                if (window.fishAudio) setTimeout(() => window.fishAudio.playWin(), 500);
              }
      ...
      for (const f of s.fish) {
        const result = f.update(dt, W, H, surfaceY, depth, s.hook, s.fish, t.schoolBehavior, s.time);
        if (result && result.hit && s.hook && s.hook.state === "set" && !s.hook.hooked) {
          s.hook.hooked = f;
      ...
      // back in by retargeting them into the visible area.
      {
        ...
        const alive = s.fish.filter(f => f.alive && f.state !== "eliminating");
        ...
        if (need > 0) {
          ...
          const offscreen = alive.filter(f => f.x < 0 || f.x > W).sort((a, b) => {
          ...
          for (const f of offscreen) {
            ...
            const entryX = fromLeft ? 40 + Math.random() * (W * 0.6) : W - 40 - Math.random() * (W * 0.6);
            f.target = {
              x: entryX,
              y: surfaceY + 50 + Math.random() * (depth - 80)
            };
            // Point them inward so they don't keep drifting out
            f.heading = fromLeft ? (Math.random() - 0.5) * 0.5 : Math.PI + (Math.random() - 0.5) * 0.5;
      ...
      drawSky(ctx, W, H, surfaceY, p, s.sun.x, s.sun.y, s.time, t.weather, s.lightning);
      ...
      // Fish
      for (const f of s.fish) f.draw(ctx, p, t.showFishOutline, s.lanternPos, t.showNames);

      // Splashes
      for (const sp of s.splashes) drawSplash(ctx, sp.x, sp.y, sp.age);
      drawFog(ctx, W, H, surfaceY, p, s.time, t.fog);
      drawBoat(ctx, W, surfaceY, s.time, t.boatDrift, p, logoRef.current, baitRef.current, s.lanternFlicker, t.lantern);
      ...
      if (t.lantern && s.lanternPos) drawLanternBeam(ctx, W, H, s.lanternPos, surfaceY, s.lanternFlicker);
      drawRain(ctx, W, H, s.time, t.rain ? t.weather === "storm" ? 1 : t.weather === "drizzle" ? 0.4 : 0 : 0);
      if (s.lightning > 0.7) {
        ctx.save();
      ...
      ctx.save();
      const vg = ctx.createRadialGradient(W / 2, H * 0.55, Math.min(W, H) * 0.32, W / 2, H * 0.55, Math.max(W, H) * 0.75);
      vg.addColorStop(0, "rgba(0,0,0,0)");
      vg.addColorStop(1, t.timeOfDay === "day" ? "rgba(0,0,0,.22)" : t.timeOfDay === "night" ? "rgba(0,0,0,.65)" : "rgba(0,0,0,.5)");
      ctx.fillStyle = vg;
      ...
      if (t.timeOfDay !== "day") {
        ...
        ctx.fillStyle = t.timeOfDay === "night" ? "rgba(80,100,120,.18)" : "rgba(120,130,140,.10)";
        ctx.fillRect(0, 0, W, H);
        ctx.restore();
      }
  ...
  }, [t.timeOfDay, t.waterDepth, t.boatDrift, t.showFishOutline, t.weather, t.fog, t.lantern, t.rain, t.showNames, t.schoolBehavior]);
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("canvas", {
    ...
    className: "brand"
  }, "Carp", /*#__PURE__*/React.createElement("em", null, "Life"), /*#__PURE__*/React.createElement("small", null, "last man standing \xB7 river beat 7")), /*#__PURE__*/React.createElement("div", {
    className: "stats"
  }, /*#__PURE__*/React.createElement("div", {
    className: "stat"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "v accent"
  }, eliminated.length)), /*#__PURE__*/React.createElement("div", {
    className: "stat"
  }, /*#__PURE__*/React.createElement("div", {
    className: "k"
  }, "Total"), /*#__PURE__*/React.createElement("div", {
    className: "v"
  }, rosterArr.length)))), /*#__PURE__*/React.createElement("div", {
    className: "roundbar"
  }, /*#__PURE__*/React.createElement("div", {
    className: "label"
  }, /*#__PURE__*/React.createElement("span", null, "Swimming"), /*#__PURE__*/React.createElement("span", {
    className: "alive"
  }, aliveCount)), /*#__PURE__*/React.createElement("div", {
    className: "sep"
  }), /*#__PURE__*/React.createElement("div", {
    className: "label"
  }, /*#__PURE__*/React.createElement("span", null, "Round"), /*#__PURE__*/React.createElement("span", {
    className: "alive"
  }, eliminated.length + 1))), /*#__PURE__*/React.createElement("div", {
    className: "catchlog"
  }, eliminated.slice(0, 6).map(e => /*#__PURE__*/React.createElement("div", {
    className: "row",
    key: e.id
  ...
  }, e.species)))), /*#__PURE__*/React.createElement("div", {
    className: `tournament ${!started || winner ? "hidden" : ""}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "countdown"
  }, countdown != null ? /*#__PURE__*/React.createElement(React.Fragment, null, "next release in ", /*#__PURE__*/React.createElement("b", null, countdown), "s") : hookBusy ? "rod in the water…" : aliveCount <= 1 ? "tournament complete" : /*#__PURE__*/React.createElement(React.Fragment, null, aliveCount, " swimming \xB7 press release")), /*#__PURE__*/React.createElement("div", {
    ...
    className: "ico"
  }, "\u2913"), " Release Rod"), /*#__PURE__*/React.createElement("label", {
    className: "auto"
  }, /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: t.autoRound,
    onChange: e => setTweak('autoRound', e.target.checked)
  }), /*#__PURE__*/React.createElement("span", null, "Auto")))), /*#__PURE__*/React.createElement("div", {
    className: `startcard ${started ? "gone" : ""}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "box"
  }, /*#__PURE__*/React.createElement("div", {
    className: "tag"
  }, "CARPLIFE COMPETITIONS \xB7 LAST MAN STANDING"), /*#__PURE__*/React.createElement("h1", null, "River ", /*#__PURE__*/React.createElement("em", null, "Fisher")), /*#__PURE__*/React.createElement("p", null, "Every fish in the river holds a name. Drop the rod at random \u2014 the first one to touch the hook is out. Eliminate until one swims alone."), /*#__PURE__*/React.createElement("label", {
    className: "field"
  }, "Participants (one per line)"), /*#__PURE__*/React.createElement("textarea", {
    value: roster,
    onChange: e => setRoster(e.target.value)
  }), /*#__PURE__*/React.createElement("button", {
    onClick: startTournament,
    disabled: rosterArr.length < 2
  }, "Begin tournament \xB7 ", rosterArr.length, " fish"))), /*#__PURE__*/React.createElement("div", {
    ...
    className: "crown"
  }, "\u25C6 LAST MAN STANDING \u25C6"), /*#__PURE__*/React.createElement("h2", null, "Champion of the river"), /*#__PURE__*/React.createElement("div", {
    className: "name"
  }, winner ? winner.name : ""), /*#__PURE__*/React.createElement("div", {
    className: "species"
  }, winner ? winner.species : "", " carp"), /*#__PURE__*/React.createElement("button", {
    onClick: startTournament
  }, "New tournament"))), /*#__PURE__*/React.createElement(TweaksPanel, {
    title: "Tweaks"
