...
function TweaksPanel({
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('message', onMsg);
    // Try to notify a host. In standalone mode (no parent host), this is a
    // no-op — but we also expose a global so a simple toolbar button can open
    // the panel without round-tripping through postMessage.
    try { window.parent.postMessage({ type: '__edit_mode_available' }, '*'); } catch (e) {}
    window.__openTweaks = () => setOpen(o => !o);
    return () => { window.removeEventListener('message', onMsg); delete window.__openTweaks; };
  }, []);
  const dismiss = () => {
    setOpen(false);
